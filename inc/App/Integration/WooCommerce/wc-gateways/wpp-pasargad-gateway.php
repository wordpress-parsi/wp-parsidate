      ...
      class WPP_WC_Pasargad_Gateway extends WC_Payment_Gateway {
        ...
        public function __construct() {
          ...
          $this->icon               = apply_filters( $this->id . '_logo',
            Assets::url( "images/woocommerce/$this->id-logo.png" ) );
        ...
        public static function send_pay_request(
          $invoice_number,
          $TerminalCode,
          $MerchantCode,
          $Amount,
          ...
          require_once( __DIR__ . '/lib/class-rsa.php' );

          $processor = new RSAProcessor( __DIR__ . '/lib/pasargad-certificate.xml', RSAKeyType::XMLFile );
          $data      = array(
        ...
        public static function check_transaction_result(
          $transaction_ref_id,
          $invoice_number = '',
          $invoice_date = '',
          $TerminalCode = '',
          ...
          $response = wp_remote_post( 'https://pep.shaparak.ir/Api/v1/Payment/CheckTransactionResult',
            $request_args );
        ...
        public static function reversal_request(
          $invoice_number,
          $invoice_date,
          $TerminalCode,
          $MerchantCode
        ...
        public static function error_response_handler( $code = '' ) {
          $error_responses = array(
            ...
            '-1550'  => esc_html__( 'It is not possible to reverse the transaction in the current state',
              'wp-parsidate' ),
            '-1549'  => esc_html__( 'The time allowed to request a transaction reversal has expired',
              'wp-parsidate' ),
            ...
            '-1531'  => esc_html__( 'The transaction was unsuccessful due to your withdrawal from the bank',
              'wp-parsidate' ),
            '-1530'  => esc_html__( 'The merchant is not authorized to confirm this transaction',
              'wp-parsidate' ),
            '-1528'  => esc_html__( 'Payment information not found', 'wp-parsidate' ),
            '-1527'  => esc_html__( 'The purchase transaction payment request operation failed',
              'wp-parsidate' ),
            ...
            '-132'   => esc_html__( 'The transaction amount is less than the minimum allowed',
              'wp-parsidate' ),
            ...
            '-111'   => esc_html__( "The transaction amount is more than the merchant's limit",
              'wp-parsidate' ),
            '-108'   => esc_html__( 'The ability to reverse the transaction is disabled for the merchant',
              'wp-parsidate' ),
            '-107'   => esc_html__( 'The ability to send transaction confirmation is disabled for the merchant',
              'wp-parsidate' ),
            ...
            '-104'   => esc_html__( 'The ability to pay the bill is inactive for the merchant',
              'wp-parsidate' ),
            ...
            '2'      => esc_html__( 'The confirmation operation of this transaction has already been done successfully',
              'wp-parsidate' ),
            ...
            '8'      => esc_html__( 'By recognizing the identity of the card holder, the transaction is successful',
              'wp-parsidate' ),
            '9'      => esc_html__( 'The received request is being followed up', 'wp-parsidate' ),
            '10'     => esc_html__( "The transaction with an amount lower than the requested amount (deficiency in the customer's account) has been accepted",
              'wp-parsidate' ),
            ...
            '20'     => esc_html__( 'In a situation where the switch needs to query the card to accept the transaction, it may make a request from the card (terminal), this message indicates that the answer is invalid.',
              'wp-parsidate' ),
            '21'     => esc_html__( 'If the response to the terminal request does not require any specific response or function, we will have this message.',
              'wp-parsidate' ),
            '22'     => esc_html__( 'The transaction was suspected of malpractice (card, terminal, card holder), so it was not accepted',
              'wp-parsidate' ),
            ...
            '32'     => esc_html__( "The transaction is incompletely completed (for example, a deposit transaction that is completed from the customer's point of view, but needs to be completed)",
              'wp-parsidate' ),
            '33'     => esc_html__( 'The card has expired', 'wp-parsidate' ),
            '38'     => esc_html__( 'The number of incorrect password entries has exceeded the limit.',
              'wp-parsidate' ),
            ...
            '57'     => esc_html__( 'The relevant transaction is not allowed by the card holder',
              'wp-parsidate' ),
            '58'     => esc_html__( "It is not allowed to perform the relevant transaction by the operator's terminal",
              'wp-parsidate' ),
            ...
            '65'     => esc_html__( 'The number of transaction requests exceeds the limit',
              'wp-parsidate' ),
            '68'     => esc_html__( 'The response required to complete or complete the transaction has arrived too late',
              'wp-parsidate' ),
            '69'     => esc_html__( 'The number of times the password has been repeated has exceeded the limit',
              'wp-parsidate' ),
            '75'     => esc_html__( 'The number of incorrect password entries has exceeded the limit',
              'wp-parsidate' ),
            '78'     => esc_html__( 'The card is not active', 'wp-parsidate' ),
            '79'     => esc_html__( 'The account connected to the card is invalid or has errors',
              'wp-parsidate' ),
            '80'     => esc_html__( 'The transaction request has been rejected', 'wp-parsidate' ),
            '81'     => esc_html__( 'The card was not accepted', 'wp-parsidate' ),
            '83'     => esc_html__( 'The switch card service provider has not accepted the transaction',
              'wp-parsidate' ),
            '84'     => esc_html__( 'In transactions that require communication with the exporter, if the exporter is not active, this message will be sent in response.',
              'wp-parsidate' ),
            '91'     => esc_html__( 'The transaction authorization system is temporarily disabled or the time set for the authorization has expired',
              'wp-parsidate' ),
          ...
          return $error_responses[ $code ] ?? esc_html__( 'The payment of the transaction was unsuccessful due to cancellation on the bank page',
            'wp-parsidate' );
        }
        ...
        public static function display_error(
          ...
          if ( 'retry' === $pay_status ) {
            ...
            $order_note = esc_html__( 'When the customer returned from the bank, the bank server did not respond, the customer was asked to refresh the page',
              'wp-parsidate' );
            $page_html  .= '<div style="color:#ff0000;font-weight:bold;font-size:12px;margin:25px">::: ' . esc_html__( 'Temporary error in payment',
                'wp-parsidate' ) . ' :::</div>
						<div style="margin-bottom:21px;font-size:12px">' . esc_html__( 'The server of the internet portal has temporarily encountered a problem, to complete the transaction moments later, click on the button below',
                'wp-parsidate' ) . '</div>
						<div style="margin:20px 0 25px 0;color:#008800" id="reqreload"><button onclick="reload_page()">' . esc_html__( 'Try again',
                'wp-parsidate' ) . '</button></div>
						<script>
							function reload_page(){
								document.getElementById("reqreload").innerHTML = "' . esc_html__( 'Trying again please wait..',
                'wp-parsidate' ) . '";
								location.reload();
							}
						</script>
            ...
            $error_messages = array(
              'reversal_done'             => array(
                ...
                'message'     => sprintf( esc_html__( 'Your payment with the tracking number %s has been successfully processed at the bank, but there has been a problem with the service',
                  'wp-parsidate' ), esc_html( $tran_id ) ),
                'description' => esc_html__( 'The order to return the money to your account has been registered in the bank, if the payment is not returned to your account within the next few hours, contact support (the maximum time to return to the account is 72 hours)',
                  'wp-parsidate' ),
                'order_note'  => esc_html__( "The customer paid the amount, but when returning from the bank, there was a problem in providing the service, the order to return the money to the customer's account was registered in the bank.",
                  'wp-parsidate' ),
              ),
              'reversal_error'            => array(
                ...
                'title'       => esc_html__( 'Service delivery error', 'wp-parsidate' ),
                /* translators: %s: Tracking number */
                'message'     => sprintf( esc_html__( 'Your payment with the tracking number %s has been successfully made in the bank, but there is a problem in providing the service!',
                  'wp-parsidate' ), esc_html( $tran_id ) ),
                'description' => esc_html__( 'The order to return the money to your account has been registered in the bank, if the payment is not returned to your account within the next few hours, contact support (the maximum time to return to the account is 72 hours)',
                  'wp-parsidate' ),
              ...
              'already_been_completed'    => array(
                ...
                /* translators: %s: Tracking number */
                'message'     => sprintf( esc_html__( 'Your order has already been successfully placed with tracking number %s',
                  'wp-parsidate' ), esc_html( $tran_id ) ),
                'description' => '',
                'order_note'  => ''
              ),
              'order_not_for_this_person' => array(
                ...
                'description' => esc_html__( 'The order number is incorrect; Call for support if needed',
                  'wp-parsidate' ),
                'order_note'  => ''
              ),
              'error_creating_order'      => array(
                ...
                'message'     => esc_html__( 'The order number is incorrect', 'wp-parsidate' ),
                'description' => esc_html__( 'There is a problem with placing an order, please contact support',
                  'wp-parsidate' ),
                'order_note'  => ''
              )
            );
            ...
            if ( array_key_exists( $pay_status, $error_messages ) ) {
              ...
              $order_note = esc_html__( 'Payment was not made',
                  'wp-parsidate' ) . ' - ' . self::error_response_handler( $pay_status );
              $page_html  .= sprintf(
                '<span %s><strong>%s</strong></span><br/><p %s>%s<br>%s</p>%s',
                $failed_color,
                $page_title,
                $desc_style,
                self::error_response_handler( $pay_status ),
                esc_html__( 'If the payment has been deducted from your bank account, it will be automatically returned to your account by the bank (the final return time to the account is 72 hours) - contact support if needed.',
                  'wp-parsidate' ),
                $back_to_checkout,
              );
            }
            ...
            if ( $order ) {
              if ( 'wc-completed' === $order->get_status() ) {
                ...
                if ( $transaction_ref_id != '' ) {
                  ...
                  $check_result = self::check_transaction_result( null, $invoice_number,
                    $invoice_date, $terminal_id, $merchant_id );
                }

                if ( isset( $check_result ) && $check_result->IsSuccess && $check_result->InvoiceNumber == $invoice_number ) {
                  $amount  = $check_result->Amount;
                  $request = self::verify_request( $invoice_number, $invoice_date, $terminal_id,
                    $merchant_id, $amount );

                  if ( isset( $request ) && $request->IsSuccess ) {
                    if ( $order->update_status( 'processing' ) ) {
                      $order->payment_complete();
                      $order->add_order_note( 'پرداخت شما با موفقیت با شماره پیگیری ' . $transaction_ref_id . ' انجام شد.',
                        1 );
                      ...
                      $reversal_request = self::reversal_request( $invoice_number, $invoice_date,
