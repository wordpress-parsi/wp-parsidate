    ...
    if ( ! class_exists( 'WC_Parsian_Gateway' ) && ! class_exists( 'WPP_WC_Parsian_Gateway' ) ) {
      ...
      class WPP_WC_Parsian_Gateway extends WC_Payment_Gateway {
        ...
        public function __construct() {
          $this->id                 = 'parsian';
          $this->gateway_name       = esc_html__( 'Parsian Bank', 'wp-parsidate' );
        ...
        public static function send_pay_request(
          ...
          if ( WcGateways::getInstance()->is_soap_enabled() ) {
            try {
        ...
        public static function display_error(
          ...
          if ( 'retry' === $pay_status ) {
            ...
            if ( array_key_exists( $pay_status, $error_messages ) ) {
              ...
              $order_note = esc_html__( 'Payment was not made', 'wp-parsidate' ) .
                            ' - ' . self::error_response_handler( $pay_status );
              $page_html  .= sprintf(
                '<span %s><strong>%s</strong></span><br/><p %s>%s<br>%s</p>%s',
                $failed_color,
        ...
        public static function redirect_to_bank( $url = '' ): void {
          if ( ! empty( $url ) ) {
            if ( headers_sent() ) {
          ...
          if ( $order ) {
            if ( $status === '0' && $token > 0 ) {
              ...
              if ( $request->Status == '0' && $request->RRN > 0 ) {
                ...
                // Add Order Note
                $order->add_order_note( sprintf( 'پرداخت با موفقیت انجام شد. کد پیگیری: %s',
                  $request->RRN ) );
