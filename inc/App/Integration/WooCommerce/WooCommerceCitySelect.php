...
class WooCommerceCitySelect {
  ...
  public function __construct() {
    $this->plugin_path = __DIR__ . '/wc-cities';
  ...
  public function form_field_city( $field, $key, $args, $value ) {
    ...
    $field = '<p class="form-row ' . esc_attr( implode( ' ',
        $args['class'] ) ) . '" id="' . esc_attr( $args['id'] ) . '_field">';

    if ( $args['label'] ) {
      $field .= '<label for="' . esc_attr( $args['id'] ) . '" class="' . esc_attr( implode( ' ',
    ...
    if ( is_array( $cities ) ) {
      ...
      foreach ( $this->dropdown_cities as $city_name ) {
        $field .= '<option value="' . esc_attr( $city_name ) . '" ' . selected( $value, $city_name,
            false ) . '>' . $city_name . '</option>';
      }
