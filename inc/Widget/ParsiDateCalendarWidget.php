...
class ParsiDateCalendarWidget extends \WP_Widget {
  public function __construct() {
    ...
    if ( version_compare( $wp_version, '4.3', '>=' ) ) {
      parent::__construct( false, __( 'Jalali Date Calender', 'wp-parsidate' ),
        'description=' . __( 'Jalali Date Calender', 'wp-parsidate' ) );
    } else {
      $this->WP_Widget( false, __( 'Jalali Date Calender', 'wp-parsidate' ),
        'description=' . __( 'Jalali Date Calender', 'wp-parsidate' ) );
    }
  }
  ...
  public function form( $instance ) {
    $title = ! empty( $instance['parsidate_calendar_title'] ) ? $instance['parsidate_calendar_title'] : __( 'Jalali Date Calender',
      'wp-parsidate' );
    ...
    <p style="text-align:right; direction:rtl">
      ...
      <input style="width:calc(100% - 120px);float:left"
             id="<?php echo $this->get_field_id( 'parsidate_calendar_title' ); ?>"
  ...
  public function widget( $args, $instance ) {
    ...
    if ( ! empty( $instance['parsidate_calendar_title'] ) ) {
      echo $args['before_title'] . apply_filters( 'widget_title',
          $instance['parsidate_calendar_title'] ) . $args['after_title'];
    }
    ...
    if ( $theme === 'dark-mode' ) {
      echo "<style>#$widget_id{background:#141414;border-radius:8px 8px 4px 4px;" .
           "overflow:hidden;box-shadow:0 0 5px 0 #000;text-align:center;padding-top:15px;color:#dcdcdc}" .
           "#$widget_id table{direction:rtl;border-radius:12px;overflow:hidden;" .
           "background:#1d1d1d;box-shadow:inset 0 0 0 6px #141414}#$widget_id table th," .
           "#$widget_id table td{border:0}#$widget_id table th:last-child," .
           "#$widget_id table tr td:last-child{color:#f28a8a}</style>";
    } else {
      echo "<style>#$widget_id{background:#dbdbdb;border-radius:12px;overflow:hidden;" .
           "box-shadow:0 0 15px 0 #0000004f,inset 0 0 0 1px #8080806e;text-align:center;padding-top:15px;" .
           "color:#1e1e1e}#$widget_id table{direction:rtl;border-radius:9px;overflow:hidden;" .
           "background:#fdfdfd;box-shadow:0 -13px 14px 0 #8080801a}#$widget_id table th," .
           "#$widget_id table td{border:0}#$widget_id table th:last-child," .
           "#$widget_id table tr td:last-child{color:#bf4a4a}</style>";
    }
  }
}
