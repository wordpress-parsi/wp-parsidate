interface ArchiveBlockAttributes {
  title: string;
  postType: string;
  type: string;
...
interface ArchiveBlockProps {
  attributes: ArchiveBlockAttributes;
  setAttributes: (attributes: Partial<ArchiveBlockAttributes>) => void;
}
...
function archiveTypeLabel(type: string): string {
  return type === 'yearly'
...
function archivePlaceholder(attributes: ArchiveBlockAttributes): unknown {
  return el(
    'div',
    { className: 'wp-block-wp-parsidate-archive__placeholder' },
    el(
      'div',
      { className: 'wp-block-wp-parsidate-archive__icon' },
      el(
        'svg',
        {
          width: 48,
    ...
    el('h3', null, attributes.title || __('Archive', 'wp-parsidate')),
    el(
      'p',
      { className: 'wp-block-wp-parsidate-archive__type' },
      archiveTypeLabel(attributes.type)
    ),
    convPermalinks
      ? null
      : el(
          'p',
          { className: 'wp-block-wp-parsidate-archive__notice' },
          __(
...
registerBlockType('wp-parsidate/archive', {
  edit(props: ArchiveBlockProps): unknown {
      ...
      el(
        ...
        el(
          ...
          el(ToggleControl, {
            ...
            onChange: (value: boolean) => setAttributes({ displaySelect: value }),
          }),
          el(ToggleControl, {
            label: __('Show post counts', 'wp-parsidate'),
            checked: attributes.displayCount,
            onChange: (value: boolean) => setAttributes({ displayCount: value }),
          })
        )
      ),
      el(
