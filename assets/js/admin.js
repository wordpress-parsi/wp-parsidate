jQuery(document).ready(function () {
    ...
    function persian_to_gregorian(jy, jm, jd) {
        var j_days_sum_month = [0, 0, 31, 62, 93, 124, 155, 186, 216, 246, 276, 306, 336, 365],
            ...
            gd = j_days_sum_month[parseInt(jm)] + parseInt(jd), gm, gy = parseInt(jy) + 621;
        if (gd > 286) gy++;
    ...
    function gregorian_to_persian(gy, gm, gd) {
        var j_days_in_month = [31, 31, 31, 31, 31, 31, 30, 30, 30, 30, 30, 29],
            ...
            day_of_year = g_days_sum_month[parseInt(gm)] + parseInt(gd),
            leab = IsLeapYear(gy),
            leap = IsLeapYear(gy - 1),
            jd, jm, jy, i;
        if (day_of_year > 79) {
    ...
    jQuery('a.edit-timestamp').on('click', function () {
        ...
        var date = gregorian_to_persian(jQuery('#aa').val(), jQuery('#mm').val(), jQuery('#jj').val()),
            div = create_stampdiv(date[0], date[1], date[2], jQuery('#hh').val(), jQuery('#mn').val());
    ...
    jQuery('#the-list').on('click', '.editinline', function () {
        ...
        if (year > 1400) {
            var month = tr.find('.mm').html(),
                ...
                minu = tr.find('.mn').html(),
                date = gregorian_to_persian(year, month, day);
            jQuery('.persian').remove();
    ...
    function change_date() {
        var old = jQuery('#timestamp b').text(),
            info = jQuery('#mma option:selected').text() + ' ' + jQuery('#jja').val() + ', ' + jQuery('#aaa').val() + ' در ' + jQuery('#hha').val() + ':' + jQuery('#mna').val();
    ...
    setTimeout(function () {
        jQuery('button.editor-post-publish-button__button').on('click', function () {
            setTimeout(function () {
                jQuery('.components-panel__body').each(function () {
                    if (!jQuery(this).attr('class').includes('jalali-calendar') && jQuery(this).find('.components-panel__body-toggle').text().includes('انتشار:'))
                        jQuery(this).hide();
                });
            }, 500);
        });
