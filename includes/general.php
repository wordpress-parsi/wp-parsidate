...
function wp_parsi_set_locale($locale) {
    ...
    if ( $settings['admin_lang'] == 'enable' ) { $admin_locale = "fa_IR"; }else{ $admin_locale = "en_US"; }
    if ( $settings['user_lang'] == 'enable' ) { $user_locale = "fa_IR"; }else{ $user_locale = "en_US"; }

    $locale = ( is_admin() ) ? $admin_locale : $user_locale;
