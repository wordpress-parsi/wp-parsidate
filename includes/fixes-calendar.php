...
function wpp_get_calendar() {
	...
	for ( $day = 1; $day <= $daysinmonth; ++ $day ) {
		...
		if ( $thisday == gmdate( 'j', ( time() + ( get_option( 'gmt_offset' ) * 3600 ) ) ) && $thismonth == gmdate( 'm',
				time() + ( get_option( 'gmt_offset' ) * 3600 ) ) && $thisyear == gmdate( 'Y',
		...
		{
			$calendar_output .= '<a href="' . get_day_link( $jthisyear, $jthismonth,
					$day ) . "\" title=\"$ak_titles_for_day[$day]\">$p_day</a>";
		} else {
