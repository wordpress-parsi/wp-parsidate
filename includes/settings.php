...
function wp_parsi_get_settings() {
	...
	if ( empty( $settings ) ) {
		update_option( 'wpp_settings', array(
			...
			'persian_date'		=>	'enable',
			'conv_title'		=>	'enable',
			'conv_contents'		=>	'enable',
			'conv_excerpt'		=>	'enable',
			'conv_comments'		=>	'enable',
			'conv_comment_count'=>	'enable',
			'conv_dates'		=>	'enable',
			'conv_cats'			=>	'enable',
			'conv_arabic'		=>	'enable',
			'conv_permalinks'	=>	'enable',
			'news_source'		=>	'parsi'
		) );
	}
...
function wpp_get_registered_settings() {
    ...
    $settings = apply_filters( 'wpp_registered_settings', array(
        'core'                  =>  apply_filters( 'wpp_core_settings', array(
            'persian_lang'      =>  array(
                'id'            =>  'persian_lang',
                'name'          =>  __( 'Persian language', 'wp-parsidate' ),
                'type'          =>  'radio',
                'options'       =>  $options,
                'std'           =>  'enable',
                'desc'          =>  __( 'This option, makes WordPress Persian and texts right to left (Currently Deactivate)', 'wp-parsidate' )
            ),
            'persian_date'      =>  array(
                'id'            =>  'persian_date',
