...
function wpp_get_tabs() {
	return array(
		...
		'plugins' => sprintf( __( '%s Plugins compability', 'wp-parsidate' ),
	...
	$settings = apply_filters( 'wpp_registered_settings', array(
		'core'    => apply_filters( 'wpp_core_settings', array(
			...
			'submenu_move' => array(
				...
				'options' => $options,
				'std'     => 'disable',
				'desc'    => __( 'With enabling this option, page item will be moved to Settings menu as submenu.',
					'wp-parsidate' )
			),
		) ),
...
function wpp_radio_callback( $args ) {
	...
	foreach ( $args['options'] as $key => $option ) :
		...
		echo '<input name="wpp_settings[' . $args['id'] . ']"" id="wpp_settings[' . $args['id'] . '][' . $key . ']" type="radio" value="' . $key . '" ' . checked( true,
				$checked, false ) . '/>';
...
function wpp_rich_editor_callback( $args ) {
	...
	if ( $wp_version >= 3.3 && function_exists( 'wp_editor' ) ) {
		$html = wp_editor( stripslashes( $value ), 'wpp_settings[' . $args['id'] . ']',
			array( 'textarea_name' => 'wpp_settings[' . $args['id'] . ']' ) );
	} else {
...
function wpp_upload_callback( $args ) {
	...
	$html .= '<span>&nbsp;<input type="button" class="wpp_settings_upload_button button-secondary" value="' . __( 'Upload File',
			'wpp' ) . '"/></span>';
	$html .= '<label for="wpp_settings[' . $args['id'] . ']"> ' . $args['desc'] . '</label>';
